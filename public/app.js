...
function renderCloudflare(cloudflare = {}) { const connected = Boolean(cloudflare.connected); const label = connected ? "Connected" : cloudflare.configured ? "Starting" : "Not configured"; const setup = cloudflare.setup || {};  $("#cloudflare-setup-badge").textContent = label;  $("#cloudflare-result").innerHTML = cloudflare.publicUrl ? `<a href="${escapeHtml(cloudflare.publicUrl)}" target="_blank" rel="noopener">${escapeHtml(cloudflare.publicUrl)}</a>` : escapeHtml(cloudflare.lastError || "Cloudflare is not connected."); $("#cloudflare-result").classList.toggle("hidden", connected); $("#cloudflare-setup-form").classList.toggle("hidden", connected); $("#advanced-cloudflare").classList.toggle("hidden", connected); $("#disconnect-cloudflare").classList.toggle("hidden", connected || !cloudflare.configured); const progress = $("#cloudflare-setup-progress"); const active = setup.state && !["idle", "complete"].includes(setup.state); progress.classList.toggle("hidden", connected || !active); $("#cloudflare-setup-message").textContent = setup.error || (setup.state === "authorized" ? "Cloudflare authorization complete. Finish creating the tunnel on this Pi." : setup.authUrl ? "Cloudflare authorization is ready. Open the authorization link below." : "Waiting for Cloudflare authorization in the browser tab…"); const link = $("#cloudflare-auth-link"); link.classList.toggle("hidden", !setup.authUrl || connected); link.href = setup.authUrl || "#"; $("#cloudflare-finish").classList.toggle("hidden", connected || setup.state !== "authorized"); const complete = $("#cloudflare-connected-row"); complete.classList.toggle("hidden", !connected); $("#cloudflare-connected-detail").textContent = cloudflare.hostname || cloudflare.publicUrl || "Cloudflare is protecting this hub."; }
function renderProvisioning(value) { state.provisioning = value; const paired = Boolean(value?.platform?.paired); const credentials = value?.credentials || {}; const visible = !paired && Boolean(credentials.oneTimeLongLivedToken); $("#provisioning-badge").textContent = paired ? "Platform paired" : "Start here"; $("#provisioning-status").textContent = paired ? `Paired with ${value.platform.apiUrl}. Token version ${value.platform.publishedVersion || 0}; local Hub Agent is ready.` : `Hub serial: ${value?.identity?.serial || "pending"}. Generate the copy values, use them in the Dinodia platform installer, then paste the returned bootstrap secret here.`; $("#provisioning-status").classList.toggle("hidden", paired); const lines = [`serial=${value?.identity?.serial || ""}`, `haBaseUrl=${value?.urls?.haBaseUrl || ""}`, `haCloudUrl=${value?.urls?.cloudUrl || "Configure the named Cloudflare hostname first"}`, `haUsername=${credentials.username || "dinodia"}`]; if (credentials.oneTimeLongLivedToken) lines.push(`haLongLivedToken=${credentials.oneTimeLongLivedToken}`); if (credentials.oneTimePassword) lines.push(`haPassword=${credentials.oneTimePassword}`); $("#provisioning-values").textContent = lines.join("\n"); $("#provisioning-values").classList.toggle("hidden", !visible); $("#regenerate-credentials").classList.toggle("hidden", paired); $("#ack-credentials").classList.toggle("hidden", paired || !visible); $("#pair-form").classList.toggle("hidden", paired); const complete = $("#provisioning-complete-row"); complete.classList.toggle("hidden", !paired); $("#provisioning-complete-detail").textContent = paired ? `Connected to ${value.platform.apiUrl || "the Dinodia platform"}. Areas and devices can now be managed.` : "This hub is connected to the Dinodia platform."; $("#permit-join").disabled = !paired; $("#commission-matter").disabled = !paired; $("#zigbee-help").textContent = paired ? "Hub provisioned. Select a Zigbee coordinator, then open pairing." : "Complete hub provisioning above before adding devices."; }
...
$("#save-token").addEventListener("click", () => { state.token = $("#token").value.trim(); localStorage.setItem("dinodia-token", state.token); connectDashboardSocket(true); load(); });
...
$("#pair-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); try { await api("/api/provisioning/pair", { method: "POST", body: JSON.stringify({ bootstrapSecret: form.get("bootstrapSecret") }) }); event.currentTarget.reset(); showNotice("Hub paired. Publishing the local home-mode token now…"); await load(); } catch (error) { showNotice(error.message, "error"); } });
$("#regenerate-credentials").addEventListener("click", async () => { if (!confirm("Generate new platform provisioning values? Previous copy values will stop working.")) return; try { await api("/api/provisioning/credentials/regenerate", { method: "POST", body: "{}" }); await load(); } catch (error) { showNotice(error.message, "error"); } });
$("#ack-credentials").addEventListener("click", async () => { try { await api("/api/provisioning/credentials/ack", { method: "POST", body: "{}" }); await load(); } catch (error) { showNotice(error.message, "error"); } });
...
$("#cloudflare-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); try { await api("/api/integrations/cloudflare", { method: "POST", body: JSON.stringify({ action: "connect", token: form.get("token"), hostname: form.get("hostname") }) }); event.currentTarget.reset(); await load(); } catch (error) { showNotice(error.message, "error"); } });
