...
function renderCloudflare(cloudflare = {}) { const localConnected = Boolean(cloudflare.connected); const verified = Boolean(cloudflare.secureAccessVerified || cloudflare.platformVerification?.state === "PLATFORM_VERIFIED"); const platformState = String(cloudflare.platformVerification?.state || (localConnected ? "LOCAL_TUNNEL_CONNECTED" : "")); const label = verified ? "Secure access verified" : localConnected ? "Tunnel connected locally" : cloudflare.configured ? "Starting" : "Not configured"; const setup = cloudflare.setup || {}; $("#cloudflare-setup-badge").textContent = label; const resultText = platformState === "PLATFORM_REPORT_FAILED" ? `Platform verification failed: ${cloudflare.platformVerification?.error || "retry required"}` : platformState === "PLATFORM_REPORT_PENDING" || (localConnected && !verified) ? "The tunnel is connected locally. Platform verification must succeed before secure access is enabled." : cloudflare.publicUrl ? `<a href="${escapeHtml(cloudflare.publicUrl)}" target="_blank" rel="noopener">${escapeHtml(cloudflare.publicUrl)}</a>` : escapeHtml(cloudflare.lastError || "Cloudflare is not connected."); $("#cloudflare-result").innerHTML = resultText; $("#cloudflare-result").classList.toggle("hidden", verified); $("#cloudflare-setup-form").classList.toggle("hidden", localConnected); $("#disconnect-cloudflare").classList.toggle("hidden", localConnected || !cloudflare.configured); $("#cloudflare-reverify").classList.toggle("hidden", !(verified && localConnected)); const progress = $("#cloudflare-setup-progress"); const active = !verified && (localConnected || (setup.state && !["idle"].includes(setup.state))); progress.classList.toggle("hidden", !active); $("#cloudflare-setup-message").textContent = platformState === "PLATFORM_REPORT_FAILED" ? "The local tunnel is still available. Retry the signed Platform verification." : platformState === "PLATFORM_REPORT_PENDING" ? "Waiting for Platform to verify the signed CloudURL report…" : setup.error || (setup.state === "authorized" ? "Cloudflare authorization complete. Finish creating the tunnel on this Pi." : setup.authUrl ? "Cloudflare authorization is ready. Open the authorization link below." : "The tunnel is connected locally; finish secure Platform verification."); const link = $("#cloudflare-auth-link"); link.classList.toggle("hidden", !setup.authUrl || verified); link.href = setup.authUrl || "#"; $("#cloudflare-finish").classList.toggle("hidden", verified || !(setup.state === "authorized" || localConnected)); $("#cloudflare-finish").textContent = localConnected ? "Retry secure verification" : "Finish tunnel setup"; const complete = $("#cloudflare-connected-row"); complete.classList.toggle("hidden", !verified); $("#cloudflare-connected-detail").textContent = cloudflare.hostname || cloudflare.publicUrl || "Cloudflare is protecting this hub."; }
...
$("#cloudflare-reverify").addEventListener("click", async () => { const button = $("#cloudflare-reverify"); button.disabled = true; button.textContent = "Verifying…"; try { await api("/api/integrations/cloudflare", { method: "POST", body: JSON.stringify({ action: "reverify" }) }); await load(); } catch (error) { showNotice(error.message, "error"); await load().catch(() => {}); } finally { button.disabled = false; button.textContent = "Verify secure access again"; } });
